  ...
  const markComplete = useMutation({
    mutationFn: async (instructionId: string) => {
      const { error } = await supabase.from("instruction_progress").upsert({
        user_id: user!.id, instruction_id: instructionId, completed: true, read_at: new Date().toISOString(),
      }, { onConflict: "user_id,instruction_id" });
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["instruction-progress"] }); toast.success(t("instructions.completed")); },
  });
    ...
    <div className="min-h-screen">
      <section className="bg-secondary/50 py-12">
        <div className="container text-center">
          <h1 className="mb-2 font-display text-3xl font-bold text-foreground">{t("instructions.title")}</h1>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {instructions.map((item: any, idx: number) => (
              <div key={item.id} className="rounded-xl border bg-card overflow-hidden transition-all hover:shadow-sm">
                <button onClick={() => setOpenId(openId === item.id ? null : item.id)}
                  ...
                  <div className="flex items-center gap-2">
                    {isCompleted(item.id) && <CheckCircle className="h-5 w-5 text-primary" />}
                    ...
                    <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                      <div className="border-s-4 border-primary mx-6 mb-6 rounded-lg bg-secondary p-6">
                        ...
                        {user && !isCompleted(item.id) && (
                          <Button size="sm" className="mt-4" onClick={() => markComplete.mutate(item.id)}>
